...
export function filterOrdersByDate<T extends { date?: string }>(orders: T[], filter: DateFilterValue): T[] {
  ...
  const thisYearStart = new Date(now.getFullYear(), 0, 1).toISOString().substring(0, 10);

  return orders.filter((o) => {
    const d = o.date?.substring(0, 10);
    if (!d) return false;
    if (filter === "today") return d >= today && d <= today;
    if (filter === "yesterday") return d >= yesterday && d <= yesterday;
